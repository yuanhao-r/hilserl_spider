...
def read_fisheye_camera(camera_index=0):
    """
    读取鱼眼相机视频流并显示
    ...
    """
    # 打开相机
    cap = cv2.VideoCapture(camera_index, cv2.CAP_V4L2)  # 使用V4L2后端，适用于Linux系统
    # 检查相机是否成功打开
    if not cap.isOpened():
    ...
    # image_crop = lambda img: img[264:677, 447:884]
    ...
    while True:
        ...
        # 显示原始鱼眼图像
        cv2.imshow('Fisheye Camera Feed', cropped_frame)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--index', type=int, default=0, help='相机索引，默认为0')
    args = parser.parse_args()
    
    read_fisheye_camera(args.index)
