...
from scipy.spatial import ConvexHull  # 用于构建凸包（需安装scipy：pip install scipy）
...
class AutoDataRecorder:
    def __init__(self, ip, output_dir, camera_indices, crop_configs=None):
        ...
        self.fixed_z = self.pos_A[2]  # Z轴固定（桌面高度，可根据需要改为动态）
        # ===================== 核心修改：不规则三维区域定义 =====================
        # 步骤1：定义不规则区域的边界点（XYZ三维坐标，可任意数量，示例为4个点构成的四边形桌面）
        # 请替换为你的实际边界点（用xArm示教器测量）
        self.boundary_points = np.array([
            [687.265564, 192.331894, self.fixed_z],  # 点1：左下
            [455.756531, 156.93837, self.fixed_z],  # 点2：右下
            [472.610046, -116.043983, self.fixed_z],   # 点3：右上
            [661.338684, -41.658051, self.fixed_z],   # 点4：左上
            # [450.0, 50.0, self.fixed_z]     # 点5：中间（可选，增加不规则性）
        ])
        
        # 步骤2：构建凸包（自动计算不规则区域的边界）
        self.hull = ConvexHull(self.boundary_points)
        self.hull_points = self.boundary_points[self.hull.vertices]  # 凸包顶点
        # 计算凸包的包围盒（用于快速采样）
        self.min_x = np.min(self.boundary_points[:, 0])
        self.max_x = np.max(self.boundary_points[:, 0])
        self.min_y = np.min(self.boundary_points[:, 1])
        self.max_y = np.max(self.boundary_points[:, 1])
    ...
    def sample_random_in_hull(self):
        """在凸包内部随机采样三维点（拒绝采样法）"""
        ...
        for _ in range(max_attempts):
            ...
            z = self.fixed_z  # Z轴固定，如需3D随机可改为：random.uniform(min_z, max_z)
            sample_point = np.array([x, y, z])
            
            # 2. 检查是否在凸包内部
            if self.is_point_inside_hull(sample_point, self.hull):
                return sample_point
        
        # 兜底：返回凸包中心
        return np.mean(self.hull_points, axis=0)
    ...
    def sample_random_target(self):
        """生成随机点（严格落在不规则三维区域内），并确保可达"""
        # 1. 在不规则区域内随机采样
        sample_xyz = self.sample_random_in_hull()
        ...
        # 3. 可达性检查 & 智能回缩（向不规则区域中心回缩）
        max_retries = 20
        original_pose = candidate_pose.copy()
        # 计算不规则区域的中心（用于回缩参考）
        region_center = np.mean(self.hull_points, axis=0)
        ...
        for i in range(max_retries):
            ...
            # 向不规则区域中心回缩
            vec_x = candidate_pose[0] - region_center[0]
            vec_y = candidate_pose[1] - region_center[1]
            
            new_target_x = region_center[0] + vec_x * 0.9  # 每次缩短10%
            new_target_y = region_center[1] + vec_y * 0.9
            
            # 确保回缩后仍在不规则区域内
            new_sample = np.array([new_target_x, new_target_y, target_z])
            if not self.is_point_inside_hull(new_sample, self.hull):
                new_sample = self.sample_random_in_hull()  # 重新采样
                new_target_x, new_target_y = new_sample[0], new_sample[1]
            
            candidate_pose[0] = new_target_x
            candidate_pose[1] = new_target_y
        
        # 兜底方案：使用不规则区域中心作为 fallback
    ...
    def run_auto_collection(self):
        ...
        print(f"采样范围：不规则三维区域（凸包顶点数: {len(self.hull_points)}）")
        print(f"区域边界点：{self.boundary_points[:, :2]} (XY平面)")
