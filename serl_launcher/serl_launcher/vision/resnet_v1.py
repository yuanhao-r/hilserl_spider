...
class ResNetEncoder(nn.Module):
    ...
    image_size: tuple = (180, 180)  # TODO: 128,128
        ...
        for i, block_size in enumerate(self.stage_sizes):
            for j in range(block_size):
                ...
                if self.use_multiplicative_cond:
                    ...
                    x = x * x_mult
        if self.pre_pooling:
            if resnet_param_fixed:
                # print("Resnet fixed !", flush=True)
                return jax.lax.stop_gradient(x)
            else:
                # print("Resnet not fixed !", flush=True)
                return x
...
class PreTrainedResNetEncoder(nn.Module):
    ...
    def __call__(
        ...
        x = observations
        if encode:
            x = self.pretrained_encoder(x, train=train, resnet_param_fixed=self.resnet_param_fixed)
